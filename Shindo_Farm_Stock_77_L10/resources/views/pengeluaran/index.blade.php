...
@section('scripts')
    <script>
        ...
        const pengeluaranBaseUrl = "{{ route('pengeluaran.index') }}";
        ...
        function muatPengeluaran() {
            ...
            fetch(`${pengeluaranBaseUrl}?bulan=${bulan}&tahun=${tahun}`, {
                    headers: {
        ...
        <tr>
          ...
          <td class="text-end">
            ...
            ${p.id === 1
              ? `<button class="btn btn-neo btn-neo-danger btn-neo-sm" disabled title="Data ini tidak bisa dihapus">Hapus</button>`
      ...
      `).join('');
        ...
        function bukaModalEdit(id) {
            fetch(`${pengeluaranBaseUrl}/${id}/edit`, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                })
        ...
        document.getElementById('formPengeluaran').addEventListener('submit', function(e) {
            ...
            const url = id ? `${pengeluaranBaseUrl}/${id}` : pengeluaranBaseUrl;
        ...
        document.getElementById('btnKonfirmasiHapusPengeluaran').addEventListener('click', function() {
            ...
            const btnHapus = document.getElementById('btnKonfirmasiHapusPengeluaran');
            btnHapus.disabled = true;
            btnHapus.textContent = 'Menghapus...';

            fetch(`${pengeluaranBaseUrl}/${idPengeluaranDihapus}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'X-Requested-With': 'XMLHttpRequest'
